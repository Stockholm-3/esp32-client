...
// -- UI elements
static lv_obj_t *temp_label, *hum_label, *pres_label;

// Helper to create the circular metric displays
static lv_obj_t *create_metric_circle(lv_obj_t *parent, const char *title,
                                      lv_obj_t **label_out) {
...
static void update_weather_cb(lv_timer_t *timer) {
    ...
    if (pres_label)
        lv_label_set_text_fmt(pres_label, "%d hPa", pres);
}
