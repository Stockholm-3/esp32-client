...
static int find_block_start(int from, float threshold, bool above, int min_slots) {
    for (int i = from; i <= 96 - min_slots; i++) {
        ...
        for (int j = 0; j < min_slots; j++) {
            bool cond = above ? (g_raw[i + j] > threshold) : (g_raw[i + j] < threshold);
            if (!cond) {
                ok = false;
                break;
            }
...
static void ui_tab_elpris_update_chart_values(const lv_coord_t* values, size_t count) {
    ...
    if (used < 96)
        memset(elpris_data + used, 0, (96 - used) * sizeof(lv_coord_t));

    lv_coord_t max_val = 0;
    for (size_t i = 0; i < 96; i++) {
        if (elpris_data[i] > max_val)
            max_val = elpris_data[i];
    }
...
static void ui_tab_elpris_update_summary(float now, float min_val, float max_val, float avg,
    ...
    if (ui_lbl_price_hi_time) {
        snprintf(buf, sizeof(buf), "at %02u:%02u", (unsigned)(max_idx / 4),
                 (unsigned)((max_idx % 4) * 15));
    ...
    if (ui_lbl_price_lo_time) {
        snprintf(buf, sizeof(buf), "at %02u:%02u", (unsigned)(min_idx / 4),
                 (unsigned)((min_idx % 4) * 15));
    ...
    lv_color_t state_color = is_good ? UI_COLOR_GOOD : is_bad ? UI_COLOR_BAD : UI_COLOR_WARN;
    if (ui_lbl_price_val)
        lv_obj_set_style_text_color(ui_lbl_price_val, state_color, LV_PART_MAIN | LV_STATE_DEFAULT);
    if (ui_lbl_price_avg)
        lv_obj_set_style_text_color(ui_lbl_price_avg, UI_COLOR_WARN,
    ...
    if (ui_lbl_elec_sub) {
        ...
        if (now_idx >= 0 && now_idx < 96) {
            ...
            if (is_good) {
                ...
                if (peak_idx >= 0)
                    snprintf(sub, sizeof(sub), "Next peak %02u:%02u", (unsigned)(peak_idx / 4),
                             (unsigned)((peak_idx % 4) * 15));
                else {
                ...
                if (cheap_idx >= 0)
                    snprintf(sub, sizeof(sub), "Cheaper from %02u:%02u", (unsigned)(cheap_idx / 4),
                             (unsigned)((cheap_idx % 4) * 15));
                else {
...
static bool ui_tab_elpris_parse_response(const char* json, size_t len) {
    ...
    float avg_diff = 1e9f;
    for (int i = 0; i < 96; i++) {
        if (i == now_idx || i == min_idx || i == max_idx)
            continue;
        float d = raw[i] - avg;
        if (d < 0)
            d = -d;
    ...
    for (int i = 0; i < 96; i++)
        chart_vals[i] = ui_tab_elpris_price_to_chart_units(raw[i]);

    ui_tab_elpris_update_summary(raw[now_idx], min_val, max_val, avg, now_idx, min_idx, max_idx,
                                 avg_idx);
...
void ui_tab_elpris_init(void) {
    ...
    lv_obj_set_flex_align(ui_panel_price_header, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START,
                          LV_FLEX_ALIGN_START);
    lv_obj_set_style_bg_opa(ui_panel_price_header, LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    ui_lbl_price_now =
        card_label(card_curr, "CURRENT PRICE", UI_COLOR_INK3, &lv_font_montserrat_12);
    ui_lbl_price_val  = card_label(card_curr, "--", UI_COLOR_GOOD, &lv_font_montserrat_40);
    ui_lbl_price_unit = card_label(card_curr, "kr/kWh", UI_COLOR_INK3, &lv_font_montserrat_14);
    ...
    elpris_series = lv_chart_add_series(ui_chart_elpris, UI_COLOR_GOOD, LV_CHART_AXIS_PRIMARY_Y);
    lv_chart_set_ext_y_array(ui_chart_elpris, elpris_series, elpris_data);

    lv_obj_add_event_cb(ui_chart_elpris, elpris_chart_draw_cb, LV_EVENT_DRAW_TASK_ADDED, NULL);
    ...
    static const char* x_ticks[] = {"00", "01", "02", "03", "04", "05", "06", "07", "08",
