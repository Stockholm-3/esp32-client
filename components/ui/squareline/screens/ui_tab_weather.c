...
static void on_weather_refresh(lv_event_t* e) {
    (void)e;
    if (g_refresh_label) lv_label_set_text(g_refresh_label, "Refreshing");
...
static void on_day_btn_clicked(lv_event_t* e) {
    ...
    if (idx == 0) {
        ...
        if (day_cnt > WEATHER_HOUR_MAX) day_cnt = WEATHER_HOUR_MAX;
        for (int m = 0; m < METRIC_COUNT; m++)
...
void ui_tab_weather_handle_server_response(const char* json, size_t len, int skip_days) {
    ...
    if (skip_days == 0) {
        ...
        for (int s = 0; s < 7; s++) { g_day_hr_start[s] = -1; g_day_hr_count[s] = 0; }
        g_hr_count = 0;
        char prev_date[11] = "";
        int  slot_day_idx  = -1;
        for (int i = 0; i < count && g_hr_count < WEATHER_HR_MAX; i++) {
            ...
            if (strncmp(date, prev_date, 10) != 0) {
                ...
                if (slot_day_idx < 7) g_day_hr_start[slot_day_idx] = g_hr_count;
            }
            int slot = slot_day_idx;

            g_hr_data[METRIC_TEMP][g_hr_count]     = cJSON_IsNumber(temp_obj)  ? (int32_t)round(temp_obj->valuedouble)      : 0;
            g_hr_data[METRIC_HUMIDITY][g_hr_count] = cJSON_IsNumber(hum_obj)   ? (int32_t)round(hum_obj->valuedouble)       : 0;
            g_hr_data[METRIC_PRECIP][g_hr_count]   = cJSON_IsNumber(prec_obj)  ? (int32_t)round(prec_obj->valuedouble * 10) : 0;
            g_hr_data[METRIC_WIND][g_hr_count]     = cJSON_IsNumber(wind_obj)  ? (int32_t)round(wind_obj->valuedouble)      : 0;
            g_hr_data[METRIC_PRESSURE][g_hr_count] = cJSON_IsNumber(press_obj) ? (int32_t)round(press_obj->valuedouble)     : 0;
            weather_timestamp_to_label(ts2, g_hr_times[g_hr_count], sizeof(g_hr_times[g_hr_count]));

            if (slot >= 0 && slot < 7) g_day_hr_count[slot]++;
            g_hr_count++;
        }
    }
    ...
    if (skip_days == 0) {
        ...
        if (di >= 0 && days[di].valid) {
            if (day_btn_arr[0]) lv_obj_remove_flag(day_btn_arr[0], LV_OBJ_FLAG_HIDDEN);
            char wday[8];
            ...
            if (day_name_arr[0]) lv_label_set_text(day_name_arr[0], wday);
            if (day_hi_arr[0])   lv_label_set_text(day_hi_arr[0], hi_buf);
            if (day_lo_arr[0])   lv_label_set_text(day_lo_arr[0], lo_buf);
            if (day_icon_arr[0]) lv_image_set_src(day_icon_arr[0],
                                  weather_code_to_icon(days[di].weather_code));
            ...
            if (day_btn_arr[0]) lv_obj_add_flag(day_btn_arr[0], LV_OBJ_FLAG_HIDDEN);
        ...
        for (int day_idx = 0; day_idx < 7 - skip_days; day_idx++) {
            ...
            if (!days[day_idx].valid) {
                if (day_btn_arr[slot]) lv_obj_add_flag(day_btn_arr[slot], LV_OBJ_FLAG_HIDDEN);
                g_day_display[slot].valid = false;
                continue;
            }
            if (day_btn_arr[slot]) lv_obj_remove_flag(day_btn_arr[slot], LV_OBJ_FLAG_HIDDEN);
            char wday[8];
            ...
            if (day_name_arr[slot]) lv_label_set_text(day_name_arr[slot], wday);
            if (day_hi_arr[slot])   lv_label_set_text(day_hi_arr[slot], hi_buf);
            if (day_lo_arr[slot])   lv_label_set_text(day_lo_arr[slot], lo_buf);
            if (day_icon_arr[slot]) lv_image_set_src(day_icon_arr[slot],
    ...
    if (skip_days == 0) {
        ...
        char name_buf[8] = "---";
        int td = today_di;
        if (td >= 0 && days[td].valid) weather_date_to_weekday(days[td].date, name_buf, sizeof(name_buf));
    ...
    if (g_refresh_label) lv_label_set_text(g_refresh_label, "Refresh");
