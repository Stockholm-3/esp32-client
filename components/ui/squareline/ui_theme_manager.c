...
inline void ui_object_set_local_style_property(lv_obj_t* object_p, lv_style_selector_t selector,
    ...
    if (object_p != NULL /*&& lv_obj_is_valid(object_p)*/) {
        lv_obj_set_local_style_prop(object_p, property, _ui_style_value_convert(property, value),
                                    selector);
    }
}

// Atomic function to set (and register) an LVGL local style-property for a given part+state
// (selector) of an object (widget) - can be used in many ways due to being atomic
void ui_object_set_themeable_style_property(lv_obj_t* object_p, lv_style_selector_t selector,
                                            lv_style_prop_t property,
...
// This function goes through all registered style-property settings and if theme is changed, sets
...
void _ui_theme_set_variable_styles(uint8_t mode) {
    ...
    for (i = 0; i < _ui_local_style_count; ++i) {
        ...
        if (style_variable_p != _ui_local_styles[i].previous_pointer ||
            ...
            property_setting_p =
                _ui_local_styles[i].style_property_settings; // first item of linked list
            for (j = 0; j < _ui_local_styles[i].style_property_setting_count; ++j) {
                if (property_setting_p->object_p != NULL) {
                    ui_object_set_local_style_property(property_setting_p->object_p,
...
lv_style_value_t _ui_style_value_convert(lv_style_prop_t property, ui_style_variable_t value) {
    ...
    // LV_STYLE_CONST_BG_OPA(1) };
    if (property == LV_STYLE_BG_COLOR || property == LV_STYLE_BG_GRAD_COLOR ||
        property == LV_STYLE_BG_IMAGE_RECOLOR || property == LV_STYLE_BORDER_COLOR ||
...
_ui_local_style_t* _ui_local_style_create(const ui_style_variable_t* style_variable_p,
                                          bool is_themeable) {
    static uint32_t i;
    static _ui_local_style_t* local_style_p;
...
// auto-update dynamic local style-array's 2nd dimension (object part+state style-property settings
// for a given style-variable)
...
_ui_local_style_property_setting_create(_ui_local_style_t* local_style_p, lv_obj_t* object_p,
    ...
    if (style_property_setting_p != NULL) { //checking might be unnecessary if count is properly
    initialized to 0 if (local_style_p->garbage_collector_couter > 0)
    --local_style_p->garbage_collector_couter; //avoid calling garbage collector every time else if
    (LV_SQUARELINE_THEME__EXPLICIT_GARBAGE_COLLECTOR_PERIOD > 0) {
            local_style_p->garbage_collector_couter =
    LV_SQUARELINE_THEME__EXPLICIT_GARBAGE_COLLECTOR_PERIOD - 1; for (i = 0; i <
    local_style_p->style_property_setting_count; ++i) { if (style_property_setting_p->object_p !=
    NULL) { //check only valid object entry (skip already emptied positions{ #if (
    defined(LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE) &&
    LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE )  //Not recommended inside explicit garbage
    collector. if ( style_property_setting_p->validcheck_counter > 0 )
    --style_property_setting_p->validcheck_counter; //avoids calling complex lv_obj_is_valid() many
    times else { //check objects with expired positive vaidity check if ( lv_obj_is_valid(
    style_property_setting_p->object_p ) ) style_property_setting_p->validcheck_counter =
    LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE; else style_property_setting_p->object_p = NULL;
    ...
    style_property_setting_p->object_p = NULL; //free up an entry of a deleted widget #endif
                }
                if (style_property_setting_p->next_p != NULL) style_property_setting_p =
    (_ui_local_style_property_setting_t*) style_property_setting_p->next_p; //get next item in
    linked-list for next round else break; //this shouldn't happen
            }
        }
    }
    #endif*/
    ...
    if (style_property_setting_p !=
        NULL) { // checking might be unnecessary if count is properly initialized to 0
        for (i = 0; i < local_style_p->style_property_setting_count; ++i) {
            if (empty_style_property_setting_p ==
                ...
                LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE )  //for slow hardware, adding this and
                setting to 5..10 in lv_conf.h might make it faster if (
                style_property_setting_p->validcheck_counter > 0 )
                --style_property_setting_p->validcheck_counter; //avoids calling complex
                lv_obj_is_valid() many times else { //check objects with expired positive vaidity
                check if ( lv_obj_is_valid( style_property_setting_p->object_p ) )
                style_property_setting_p->validcheck_counter =
                LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE; else { //free up an entry of a deleted
                widget style_property_setting_p->object_p = NULL; empty_style_property_setting_p =
                style_property_setting_p;
                        }
                    }
                    #else
                    if ( !lv_obj_is_valid( style_property_setting_p->object_p ) ) { //free up an
                entry of a deleted widget style_property_setting_p->object_p = NULL;
